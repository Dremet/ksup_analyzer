from csup_analyzer.event.Result import RaceResultsDataFrame
...
class LapPositionTablePlot:
    ...
    def plot(self, fileprefix = None):
        ...
        if fileprefix == None:
            fig.savefig(f"export//{dt.strftime('%d_%m_%Y_%H_%M_%S')}_LapPosition.png", dpi=150)
        else:
...
class GapToWinnerTablePlot:
    ...
    def plot(self, ymin = -15, ymax = 15, fileprefix = None):
        ...
        dt = self.race.datetime_race
        if fileprefix == None:
            fig.savefig(f"export//{dt.strftime('%d_%m_%Y_%H_%M_%S')}_GapToWinner.png", dpi=150)
        else:
...
class GapToLeaderTablePlot:
    ...
    def plot(self, ymin = -1, ymax = 30, fileprefix = None):
        ...
        dt = self.race.datetime_race        
        if fileprefix == None:
            fig.savefig(f"export//{dt.strftime('%d_%m_%Y_%H_%M_%S')}_GapToLeader.png", dpi=150)
        else:
